 ...
 * - Numerical Order (Windows CE 5.0)" reference (aa450919; the full
 * row set, 0 through 6118, was captured across M1..M25).  The numbers
 * are the Win32 error-code ABI values (identical across desktop
 * Windows and Windows CE); rows that the CE table itself omits are
 * intentionally not defined here.  Constants first cited by
 * individual CE function pages carry extra page-ID comments below.
 */
...
/*
 * M25: system-error rows of the official CE page aa450919
 * "System Errors - Numerical Order (Windows CE 5.0)" not captured
 * above (codes through 6118; alphabetical names aa450917).  Fixed
 * Win32-ABI error values.
 */
...
/*
 * M25: official CE 5.0 error pages for the cryptographic and
 * trust namespaces: NTE error codes aa451033, CRYPT error codes
 * aa451026, CERTSRV error codes aa451023, TRUST error codes
 * aa451035.  Hex values are printed verbatim on the pages (fixed
 * Win32-ABI codes).  The RAS page (ms886792) defines its values
 * only as "RASBASE+n" expressions and the Net-errors pages
 * (ms886761/ms886762) print NERR_ values that conflict with the
 * desktop Net Error Codes reference (e.g. NERR_LastAdmin 242 vs
