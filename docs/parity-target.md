...
| OEM keyboard / virtual keys: `keybd.h`/`pwinuser.h` -- KEY_STATE_FLAGS (values held), GET_FOREGROUND_INFO + the five Pwinuser.h functions, PostKeybdMessage (Winuser.h home), the full CE 5.0 VK_* tables (~140 published constants incl. OEM/DBE/manufacturer sets) | shipped (M52) |
| Standard Shell Reference: `shtypes.h`/`shobjidl.h`/`cpl.h`/`shelwapi.h`/`windowsx.h`/`commdlg.h` (new) + `shellapi.h`/`shlobj.h`/`winuser.h` extended -- the 82-page CE 5.0 book (folder browsing/PIDL, notify icons, ShellExecuteEx, file ops, shortcuts, CPL applets, the six Standard Shell interfaces as M44-style records, SHGNO); published CSIDL (15) + SFGAO (25) value tables; WM 6.5 gesture supplement: NMRGINFO + GN_CONTEXTMENU (aygshell.h), NM_RECOGNIZEGESTURE record (commctrl.h) | shipped (M53) |
| Derived-value policy: unpublished constants implemented only when uniquely derivable from official public information (derivation paths recorded in docs/inventory.md) | adopted (M52): SIPSTATE + SHSipPreference, SHIC_FEATURE, VK_APP1-6, VK_KEYLOCK, KEYEVENTF_EXTENDEDKEY/KEYUP, INPUT_MOUSE/KEYBOARD/HARDWARE; sources: WM 6.5 official CHM (pageswm/) + desktop Win32 references (pagesw/) |
...
| Import-library defs (`def/*-doc.def`) | doc-derived only — `tools/ce-fetch.py` harvests per-page Link Library rows into `build/rows.json`; `tools/gen-doc-def.py` writes the committed defs; `llvm-dlltool` builds the import libs. Current: coredll **619** (incl. the 89 M45 TAPI client functions + TSPI_lineForward, the 42 M47 Crypt*, the 9 M51 Sip* functions, KeybdGetDeviceInfo, and the 7 M53 Standard Shell names), coredll-adjacent (coreloc, commctrl, msgque, ...), ws2 **100**, **ole32 84**, **oleaut32 222**, coreimm **56**, crypt32 **83**, winscard **28**, aygshell **34**, kbdui **8**, **ceshell 9**, **shmisc 1** (M53) |
| Real-toolchain compile matrix (headers + TU, `-Werror`) | passing: `make crosscheck WINCECLANG=...` for arm/i386 × CE 4.2/5.0/6.0 (all 47 headers standalone + full TU) |
| End-to-end links (wince-crt + wince-api consumer, against doc-derived import libraries) | shipped (M37+, extended M39–M44, M49–M53): `make e2e` links the tests/e2e consumers (main app / WinMain app / DLL) on all six arm/i386-pc-wince{4.2,5.0,6.0} targets; PE machine/CE-subsystem/import names asserted for coredll.dll (incl. the M51 Sip* names, KeybdGetDeviceInfo and the M53 ShellExecuteEx/SHGetSpecialFolderPath), ws2.dll, ole32.dll/oleaut32.dll, crypt32.dll, winscard.dll, aygshell.dll (incl. SHSipPreference), kbdui.dll and ceshell.dll/shmisc.dll (M53) |
| Queued (page-grounded, official pages in corpus) | remaining CE books not yet harvested in full (CE 4.2 book pages, Common Controls book), other second-order DLLs, the held Winsock/TSPI/IMM/crypto/smart-card/AYGShell/SIP/keyboard constant sets (names published without values; on-device readback owed, or a future official-source derivation under the M52 policy) |
