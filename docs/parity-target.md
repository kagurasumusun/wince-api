...
| Import-library defs (`def/*-doc.def`) | doc-derived only — `tools/ce-fetch.py` harvests per-page Link Library rows into `build/rows.json`; `tools/gen-doc-def.py` writes the committed defs; `llvm-dlltool` builds the import libs. Current: coredll **612** (incl. the 89 M45 TAPI client functions + TSPI_lineForward, the 42 M47 Crypt*, the 9 M51 Sip* functions and KeybdGetDeviceInfo, M52), coredll-adjacent (coreloc, commctrl, msgque, ...), ws2 **100**, **ole32 84**, **oleaut32 222**, coreimm **56**, crypt32 **83**, winscard **28**, aygshell **34**, kbdui **8** |
| Real-toolchain compile matrix (headers + TU, `-Werror`) | passing: `make crosscheck WINCECLANG=...` for arm/i386 × CE 4.2/5.0/6.0 (all 39 headers standalone + full TU) |
| End-to-end links (wince-crt + wince-api consumer, against doc-derived import libraries) | shipped (M37+, extended M39–M44, M49–M52): `make e2e` links the tests/e2e consumers (main app / WinMain app / DLL) on all six arm/i386-pc-wince{4.2,5.0,6.0} targets; PE machine/CE-subsystem/import names asserted for coredll.dll (incl. the M51 Sip* names and KeybdGetDeviceInfo), ws2.dll, ole32.dll/oleaut32.dll, crypt32.dll, winscard.dll, aygshell.dll (incl. SHSipPreference) and kbdui.dll |
